...
def check_rpm_availability(container: str, cgo_info: Dict) -> Dict:
    """
    对 CGO 扫描结果中的所有依赖，在容器内批量查询 RPM 可用性。
    """
    print(f"\n[INFO] 在容器 {container} 内查询 RPM 可用性（单次批量查询）...")
    tasks = build_lookup_tasks(cgo_info)

    try:
        results = run_batch_lookup(container, tasks, timeout=300)
...
def main():
    ...
    if args.check_rpm and not args.container:
        print("[ERROR] --check-rpm 需要指定 --container <容器名>", file=sys.stderr)
        sys.exit(1)
    ...
    if args.check_rpm:
        if not cgo_info["has_cgo"]:
            ...
            rpm_check = check_rpm_availability(args.container, cgo_info)
