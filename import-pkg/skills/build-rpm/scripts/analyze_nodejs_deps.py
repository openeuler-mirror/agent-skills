...
def check_rpm_availability(container: str, link_libs: List[str]) -> Dict:
    ...
    print(f"\n[INFO] 在容器 {container} 内查询原生扩展 RPM 可用性...")
...
def check_runtime_deps(container: str, dependencies: Dict[str, str]) -> Dict:
    """
    ...
    for npm_name, version_constraint in dependencies.items():
        ...
        try:
            results = run_batch_lookup(container, tasks, timeout=60)
...
def main():
    ...
    if args.check_rpm:
        ...
        runtime_deps = check_runtime_deps(args.container, pkg_info.get("dependencies", {}))
