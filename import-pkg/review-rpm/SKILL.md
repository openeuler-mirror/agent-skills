---
name: review-rpm
description: RPM 包引入事后反馈：构建完成（成功或失败）后对整个引入过程复盘，提炼可复用经验写入 lessons 文件，生成结构化反馈和最终汇总报告。由 pkg-introduce 在流程结束后通过 /review-rpm 调用，不阻断构建流程。build-rpm 的 review-fix 循环在 rpmbuild 成功后调用 critique stage 获取结构化修复指令。
